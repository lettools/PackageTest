...
ensemblAttributes<-function(spec="hsapiens", vers=NULL) {
  ...
  while( is.null(ens) && attempt < maxAttempts ) {
    ...
    tryCatch({
      ...
      if(!is.null(ens))
      {
        cat("Connection successful. Getting gene info.\n")
        cat("This can take a few minutes.\n")
        ensAttr <- getBM(attributes=c("chromosome_name", "ensembl_transcript_id", "ensembl_gene_id", "start_position", "exon_chrom_start", "exon_chrom_end", "transcript_start" , "transcript_end", "strand"), mart
                         =ens)
        print("Completed retrieving gene info.\n")
